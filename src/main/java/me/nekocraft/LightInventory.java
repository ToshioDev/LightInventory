package me.nekocraft;

import me.nekocraft.gui.content.InventoryContents;
import me.nekocraft.gui.content.InventoryProvider;
import me.nekocraft.gui.InventoryListener;
import me.nekocraft.gui.InventoryManager;
...
public class LightInventory {
    ...
    private LightInventory parent;

    private List<InventoryListener<? extends Event>> listeners;
    private InventoryManager manager;

    private LightInventory(InventoryManager manager) {
        this.manager = manager;
    }

    public Inventory open(Player player) { return open(player, 0); }
    public Inventory open(Player player, int page) {
        Optional<LightInventory> oldInv = this.manager.getInventory(player);
    ...
    public Optional<LightInventory> getParent() { return Optional.ofNullable(parent); }
    ...
    public static final class Builder {
        ...
        public Builder parent(LightInventory parent) {
            this.parent = parent;
            return this;
        }
        ...
        public LightInventory build() {
            if(this.provider == null)
                throw new IllegalStateException("The provider of the LightInventory.Builder must be set.");

            if(manager == null)
                throw new IllegalStateException("The manager of the LightInventory.Builder must be set");

            LightInventory inv = new LightInventory(manager);
            inv.id = this.id;
